/* ============================================================
   ...
   - Pixel-art (dot matrix) pour les vignettes produits
...
/* ---- PIXEL-ART : des bitmaps "X" rendus en SVG ---- */
var PIX = {
  LAMP: [
    "  ######   ",
    " ######    ",
...
function renderPix(el) {
  var src = el.hasAttribute('data-pix') ? el : (el.closest('[data-pix]') || {});
  var rows = PIX[src.getAttribute('data-pix')];
  var color = src.getAttribute('data-color') || '#58a6ff';
  if (!rows) return;
  var w = (rows[0] || '').length;
  var h = rows.length;
  var rects = '';
